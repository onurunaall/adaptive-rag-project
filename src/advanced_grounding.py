...
class AdvancedGroundingCheck(BaseModel):
    ...
    factual_claims: List[str] = Field(
        description="List of specific factual claims made in the answer"
    )
    ...
    unsupported_claims: List[str] = Field(
        default_factory=list, 
        description="Claims that lack support in the context"
    )
    
    partially_supported_claims: List[str] = Field(
        default_factory=list,
        description="Claims that have some but incomplete support"
    )
    ...
    citation_accuracy: float = Field(
        description="Score from 0.0 to 1.0 indicating accuracy of any citations or references"
    )
    ...
    context_coverage: float = Field(
        description="Score indicating how well the answer covers the available context (0.0-1.0)"
    )
...
class ConsistencyCheck(BaseModel):
    ...
    contradictions_found: List[str] = Field(
        default_factory=list,
        description="Any contradictions found within the answer"
    )
...
class CompletenessCheck(BaseModel):
    ...
    addresses_sub_questions: List[str] = Field(
        default_factory=list,
        description="Sub-questions or aspects that are addressed"
    )
...
class MultiLevelGroundingChecker:
    ...
    async def perform_comprehensive_grounding_check(
        ...
        try:
            ...
            # 2. Consistency check
            consistency_result = await self.consistency_chain.ainvoke({
                "answer": answer
            })
            ...
            hallucination_result = await self.hallucination_detector_chain.ainvoke({
                "context": context,
                "answer": answer
            })
    ...
    def _calculate_overall_assessment(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            is_acceptable = (
                ...
                (not detailed or detailed.is_fully_grounded) and
                (not consistency or consistency.is_internally_consistent) and
                (not hallucination or not hallucination.get("hallucinations_found"))
            )
